...
$(document).ready(function() {
    ...
    var dataString="email="+email+"&password="+password+"&account_type="+account_type+"&signup=";
    if($.trim(email).length>0 & $.trim(password).length>0 & $.trim(account_type).length>0)
    {
        $.ajax({
          type: "POST",
          url: url,
      ...
      if($.trim(id).length>0 & $.trim(first_name).length>0 & $.trim(insta_username).length>0)
            {
                $.ajax({
                    ...
                    crossDomain: true,
                    cache: false,
                    beforeSend: function(){ $("#update_account_in").val('Connecting...');},
                    success: function(data)
                    {
      ...
      if($.trim(id).length>0 & $.trim(first_name).length>0 )
            {
                $.ajax({
                    ...
                    crossDomain: true,
                    cache: false,
                    beforeSend: function(){ $("#update_account_bs").val('Connecting...');},
                    success: function(data)
                    {
                    var obj = JSON.parse(data);
                    var current_status = obj.status;
